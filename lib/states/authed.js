...
var startMovieIntent = function(request, response) {
    var app = request.data._plex_app;
    var movieName = request.slot('movieName', null);

    if(!movieName) {
        // TODO ask for which show
        response.say("No movie specified");
        return response.send();
    }

    plexutils.startMovie(app, {
        playerName: app.user.player.product,
        spokenName: movieName
    }, response).then(function() {
        response.send();
    }).catch(function(err) {
        app.skill.error(err, request, response);
...
var yesIntent = function(request,response) {
    ...
    if(promptData.yesAction === 'startEpisode' || promptData.yesAction === 'startMovie') {
...
module.exports = {
    intents: {
        ...
        'StartMovieIntent': startMovieIntent,
