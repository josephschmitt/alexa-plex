...
var startMovie = function(app, options, response) {
    if(!options.spokenName) {
        return Q.reject(new Error('startMovie must be provided with a spokenName option'));
    }
    if(!options.playerName) {
    ...
    var playerName = options.playerName;
    var spokenName = options.spokenName;
    var movie;
    var releaseDate;
    var duration;

    return search(app, spokenName, 'movie').then(function(results) {
        movie = results[0];
        if (movie) {
            releaseDate = moment(movie.originallyAvailableAt);
            duration = moment.duration(movie.duration);

            response.session('promptData', {
                yesAction  : 'startMovie',
                yesResponse: 'Starting ' + movie.title + ', on ' + playerName,
                mediaKey   : movie.key,
                noResponse : 'Oh. Sorry about that.',
                noAction   : 'endSession'
            });
            response.shouldEndSession(false);
            return response.say("Start watching " + movie.title + " (" + movie.year + ")?")
                .card(movie.title + " (" + movie.year + ")", movie.summary +
                    "\n\n" + duration.hours() + "h " + duration.minutes() + "m  Rating: " + movie.rating +
                    "  Released: " + releaseDate.format('LL') + "  Studio: " + movie.studio);
        } else {
            response.shouldEndSession(true);
            return response.say("Sorry, no movies found matching \"" + spokenName + "\"")
                .card("No Movie Found", spokenName);
        }
...
/**
 * Searches based on a search term and optionally limits by type (movie or show).
 * @param  {App-App} app
 * @param  {String} term
 * @param  {String} type -- "movie" or "tv show"
 * @return {Promise}
 */
var search = function(app, term, type) {
    return query(app.plex.pms, '/search?query=' + term)
        .then(function(results) {
            if (results && results._children) {
                return results._children.filter(function(result) {
